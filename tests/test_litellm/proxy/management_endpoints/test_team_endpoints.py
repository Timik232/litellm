...
async def test_reset_team_member_spend_fn_success(monkeypatch):
    ...
    assert real_spend_counter_cache.in_memory_cache.get_cache(key="spend:team_member:member-1:team-1") is None
...
async def test_team_member_update_invalidates_team_member_spend_state_when_budget_patch_applied(monkeypatch):
    ...
    reaches the admission check and the member stays 429ing. Asserted against real cache
    reads, not mock call args, so a change that keeps the call but drops its effect still
    fails."""
    from litellm.caching.dual_cache import DualCache
    from litellm.proxy.common_utils.user_api_key_cache import UserApiKeyCache
